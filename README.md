...
- `interlocking/` — Interlocking Squares puzzle (diamond-shaped grid with shared corner letters)
...
A ready-to-print puzzle and its solution, generated by `interlocking/interlocking.py`:
...
## Interlocking Squares (`interlocking/interlocking.py`)

A diamond-shaped grid of six rows (widths 3·5·7·7·5·3). Interior corners shared by adjacent rows form 4-letter square words that wind clockwise or anticlockwise from any corner. Each puzzle has 18 such squares, clued by colour region (green / blue / red).

### Features

- **CSP solver** with forward-checking and quality scoring
- **Acrostic** (`--acrostic WORD`) — constrains row first letters to spell a hidden message, highlighted in crimson on the solution
- **Theme words** (`--include "word; word"`) — the solver prioritises grids containing your chosen words as row words or square words
- **PNG output** — solution and puzzle images with clue panels, generated via Claude (requires API key)
- **Row clueing** (`--rows B D`) — choose which rows receive explicit across clues; others are left for the solver to deduce

### Quick start

```bash
# Basic puzzle (random seed)
python3 interlocking/interlocking.py --seed 850 --png out.png

# Puzzle with 6-letter acrostic hidden message, rows B and D clued
python3 interlocking/interlocking.py --seed 3100 --min-score 60 \
    --acrostic ENIGMA --rows B D \
    --title '"Puzzling" Interlocking Squares' \
    --png interlocking/images/isq_enigma.png

# Include theme words (e.g. a name as a square word)
python3 interlocking/interlocking.py --seed 1 \
...
--time-limit F      Seconds per search attempt (default 60)
--acrostic WORD     Constrain first letters of rows to spell WORD (3–6 letters)
--rows A B …        Which rows to clue explicitly (e.g. B D)
--include "w;w;w"   Theme words to try to include (rows or square words)
--min-included N    Require at least N theme words in the solution
--col N             Pre-fill column N in the puzzle image
--title TEXT        Override default puzzle title
